...
const Ready = () => {
    ...
    const data = useNotCategory('finished') || [];
    const finishedData = useCategory('finished') || [];
    const dispatch = useDispatch();
    const handleButtonClick = () => {
        setButtonActive(true);
    };
    const handleTaskClick = (id) => {
        dispatch(updateTaskStatus({ id, status: 'finished' }));
        setButtonActive(false);
    };
    const isDisabled = data.length === 0;
    return (_jsxs("div", { className: 'rounded-[10px] bg-[#EBECF0] h-auto p-3 flex justify-between flex-col gap-4 relative', children: [_jsxs("div", { children: [_jsx("h2", { className: 'text-[18px] font-bold', children: "Finished " }), _jsx("ul", { className: 'flex flex-col gap-4', children: finishedData &&
                            finishedData.map((e) => (_jsx(Link, { to: `/task/${e.id}`, children: _jsx("li", { className: 'cursor-pointer text-[18px] bg-white p-2 rounded-md ', onClick: () => handleTaskClick(e.id), children: e.title }) }, e.id))) })] }), buttonIsActive && (_jsx("div", { className: 'bg-white shadow-md rounded-md h-[100px] p-2 overflow-y-scroll absolute bottom-10 left-[50%] w-[90%] translate-x-[-50%] z-10 bg-slate-400', children: _jsx("ul", { children: data &&
